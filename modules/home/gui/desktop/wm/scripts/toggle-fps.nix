...
let
  ...
  mkMonitor =
    refreshRate: # sh
    ''
      ${hyprctl} keyword monitor '${my.gui.display.name}, ${my.gui.display.string.width}x${my.gui.display.string.height}@${refreshRate}Hz, auto, ${my.gui.display.string.scale}'
    '';
in
{
  home.packages = [
    (pkgs.writeShellScriptBin "my-toggle-fps" ''
      function get_rr() {
