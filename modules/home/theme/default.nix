...
let
  cfg = config.theme;
  themePkgs = inputs.catppuccin.packages.${pkgs.stdenv.hostPlatform.system};
in
{
  imports = [
  ...
  options.theme = {
    ...
    color = lib.mkOption {
      ...
      default =
        ...
        }) (builtins.fromJSON (builtins.readFile "${themePkgs.palette}/palette.json")).${cfg.flavor}.colors;
    };
  };
    ...
    _module.args = {
      mkGetTheme =
        ...
        let
          ...
          output = if package != null then "${themePkgs.${package}}/${formattedName}" else formattedName;
        in
