  ...
  mkStyledText =
    {
      text,
      bold ? false,
      color ? config.my.theme.color.text,
    }:
...
in
{
  home.packages = [
    (pkgs.writeShellScriptBin "notifybar" ''
      ...
      'Charging') battery_status_color="${config.my.theme.color.green}" ;;
      'Discharging') battery_status_color="${config.my.theme.color.maroon}" ;;
      esac
      if [ $battery_capacity -gt 80 ]; then
        battery_capacity_color="${config.my.theme.color.maroon}"
      elif [ $battery_capacity -gt 50 ]; then
        battery_capacity_color="${config.my.theme.color.green}"
      elif [ $battery_capacity -gt 20 ]; then
        battery_capacity_color="${config.my.theme.color.yellow}"
        battery_title_style="bold"
      else
        battery_capacity_color="${config.my.theme.color.red}"
        battery_title_style="bold"
      fi
      ...
      if [ $brightness_status -gt 80 ]; then
        brightness_color="${config.my.theme.color.red}"
        brightness_title_style="bold"
      elif [ $brightness_status -gt 50 ]; then
        brightness_color="${config.my.theme.color.maroon}"
        brightness_title_style="bold"
      elif [ $brightness_status -gt 20 ]; then
        brightness_color="${config.my.theme.color.yellow}"
      else
        brightness_color="${config.my.theme.color.green}"
      fi
      ...
      if [ $caffiene_inactive -eq 1 ]; then
        ...
        caffiene_color="${config.my.theme.color.overlay0}"
      else
        caffiene_status="ENABLED"
        caffiene_title_style="bold"
        caffiene_color="${config.my.theme.color.peach}"
      fi
      ...
      if [ $audio_mute -eq 0 ]; then
        ...
        audio_color="${config.my.theme.color.red}"
      else
        audio="$audio_volume"
        if [ $audio_volume -gt 80 ]; then
          ...
          audio_color="${config.my.theme.color.maroon}"
          audio_title_style="bold"
        elif [ $audio_volume -gt 20 ]; then
          audio_color="${config.my.theme.color.yellow}"
        else
          audio_color="${config.my.theme.color.green}"
        fi
      fi
      ...
      if [ $mic_mute -eq 0 ]; then
        mic="MUTED"
        mic_color="${config.my.theme.color.green}"
      else
        mic="UNMUTED"
        mic_title_style="bold"
        mic_color="${config.my.theme.color.red}"
      fi
